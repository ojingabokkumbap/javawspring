...
<p><br/></p>
  ...
  <table class="table">
    ...
    <tr><td>생일: ${fn:substring(vo.birthday,0,fn:length(vo.birthday)-10)}</td></tr>
    ...
    <tr><td>최초 가입일 : ${fn:substring(vo.startDate,0,fn:length(vo.startDate)-5)} </td></tr>
    <tr><td>마지막 방문일 : ${fn:substring(vo.lastDate,0,fn:length(vo.lastDate)-5)}</td></tr>
    <tr><td>오늘 방문횟수: ${vo.todayCnt}</td></tr>
    <c:if test="${sLevel == 0}">
	    <tr><td>총 포인트: ${vo.point}</td></tr>
	    <tr><td>공개유무: ${vo.userInfor}</td></tr>
    </c:if>
    <tr><td>사진 : <img src="${ctp}/member/${vo.photo}" width="200px"/></td></tr>
